    ...
    public class loginModel : PageModel
    {
        ...
        public IActionResult OnPost()
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                string query = "SELECT * FROM Login WHERE UserName = @Username AND UserPassword = @Password";
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            string userId = reader["UserId"].ToString(); // Replace 'UserId' with your column name
                            ViewData["Id"] = userId;
